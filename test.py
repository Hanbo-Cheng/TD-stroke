...
valid_datasets = [bfs2_path + 'test_images.pkl', bfs2_path + 'test_labels.pkl', bfs2_path + 'test_relations.pkl']
...
for kk, vv in reworddicts.items():
    reworddicts_r[vv] = kk
# load valid gtd
with open(bfs2_path + 'test_labels.pkl', 'rb') as fp:
...
WAP_model.eval()
ud_epoch = time.time()
fp_results = open('reuslts-test.txt', 'w')
with torch.no_grad():
    valid_count_idx = 0
