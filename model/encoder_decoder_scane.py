...
from .wap_encoder import WAP_Encoder
from .tap_encoder import TAP_Encoder
from .decoder_scan import Decoder
import math
import copy
...
class Encoder_Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.tap_encoder = TAP_Encoder(growthRate=params['growthRate'], reduction=params['tap_reduction'],
                                       bottleneck=params['tap_bottleneck'],
                                       use_dropout=params['tap_densenet_use_dropout'],
                                       n_feature=params['tap_n_features'], hidden_size=params['hidden_size'])
        self.wap_encoder = WAP_Encoder(growthRate=params['growthRate'], reduction=params['wap_reduction'],
                                       bottleneck=params['wap_bottleneck'], use_dropout=params['use_dropout'])
        self.init_context = nn.Linear(params['tap_D'] + params['wap_D'], params['n'])
    ...
    def forward(self, params, off_x, on_x, off_x_mask, on_x_mask,  
                 off_stroke_mask, on_stroke_mask, C_y,  
                P_y, C_re, P_re, y_mask, re_mask, length):

        #encoder
        tap_stroke_ctx, tap_stroke_ctx_mask = self.tap_encoder(on_x, on_x_mask, on_stroke_mask)
        wap_stroke_ctx, wap_stroke_ctx_mask = self.wap_encoder(off_x, off_x_mask, off_stroke_mask)

        stroke_ctx = torch.cat((tap_stroke_ctx, wap_stroke_ctx), 2) # seqlen, batch, channels
        stroke_ctx_mask = tap_stroke_ctx_mask   #
        stroke_ctx_mean = (stroke_ctx * stroke_ctx_mask[:, :, None]).sum(0) / stroke_ctx_mask.sum(0)[:, None]

        init_state = torch.tanh(self.init_context(stroke_ctx_mean))  # (batch,n)

        #decoder
        predict_objects, predict_relations, predict_objects_pix = self.decoder(stroke_ctx, stroke_ctx_mask, 
    ...
    def greedy_inference(self, off_x, on_x, off_x_mask, on_x_mask,  
                 off_stroke_mask, on_stroke_mask, max_length, p_y, p_re, p_mask):
        #encoder
        tap_stroke_ctx, tap_stroke_ctx_mask = self.tap_encoder(on_x, on_x_mask, on_stroke_mask)
        wap_stroke_ctx, wap_stroke_ctx_mask = self.wap_encoder(off_x, off_x_mask, off_stroke_mask)
        ...
        ctx = stroke_ctx.permute(1,2,0)[:,:,None,:]
        B,_, H, W = ctx.shape

        ctx_mask = stroke_ctx_mask.permute(1,0)
    ...
    def greedy_inference_stack(self, off_x, on_x, off_m, on_m, x_mask, max_length, p_y, p_re, p_mask):
        
        tap_stroke_ctx, tap_stroke_ctx_mask = self.tap_encoder(on_x, on_x_mask, on_stroke_mask)
        wap_stroke_ctx, wap_stroke_ctx_mask = self.wap_encoder(off_x, off_x_mask, off_stroke_mask)

        stroke_ctx = torch.cat((tap_stroke_ctx, wap_stroke_ctx), 2)
        stroke_ctx_mask = tap_stroke_ctx_mask
        stroke_ctx_mean = (stroke_ctx * stroke_ctx_mask[:, :, None]).sum(0) / stroke_ctx_mask.sum(0)[:, None]

        ht = torch.tanh(self.init_context(stroke_ctx_mean)) 
    ...
    def beamsearch_inference(self, x, x_mask, max_len, p_y, p_re, p_mask, beam_sise = 10):
        # init state
        ...
        for ei in range(max_len):
            score, ht, attention,ct   = self.decoder.get_child(ctx_input, 
