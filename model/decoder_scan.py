...
class Decoder(nn.Module):
    ...
    def forward(self, ctx_val, ctx_mask, C_ys, P_ys, P_y_masks, 
        ...
        ctx_mask = ctx_mask.permute(1,0)
        B, Seqlen  = ctx_mask.shape
        # Seqlen, B = ctx_mask.shape
        ctx_val = ctx_val.permute(1,2,0)[:,:,None,:]  # batch, channels , 1,seqlen
        #attentions = torch.zeros(length, B, Seqlen).cuda()
        attention_past = torch.zeros(B, 1 ,1, Seqlen).cuda()
        ...
        predict_features = self.get_predicts(ctx_val) #(Batch, seqlen, k)
...
class CoverageAttention(nn.Module):
    def __init__(self, params):
        ...
        self.conv_att_past = nn.Conv2d(1, 512, kernel_size=(11,1), bias=False, padding=(5,0))
    ...
    def forward(self, ctx_val, ctx_key, ctx_mask, attention_past, ht_query):
        ...
        # attention_past = attention_past[:,:,None,:]
        ...
        attention_score = torch.tanh(ctx_key + ht_query[:, None, None, :] + attention_past)   # batch,1,seqlen.channels
        attention_score = self.fc_attention(attention_score).squeeze(3)
        
        attention_score = attention_score - attention_score.max()
        attention_score = torch.exp(attention_score) * ctx_mask[:,None,:]
