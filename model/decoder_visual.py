...
class Decoder(nn.Module):
    ...
    def forward(self, ctx_val, ctx_mask, C_ys, P_ys, P_y_masks, 
        ...
        #ctx_val_pool = nn.functional.avg_pool2d(ctx_val, (2,2), stride=(2,2), ceil_mode=True)
        ...
        return predict_childs, predict_relations
    ...
    def get_relation(self, ctx_val, ctx_key, ctx_mask, c_y, ht, ct):
        ...
        ct, attention = self.relation_attention(ctx_val, ctx_key, ctx_mask, ht_query)
        ht = self.gru11(ct, ht_query)

        predict_relation = self.relation_probility(ct, c_y, ht)
        return predict_relation, ht, attention
