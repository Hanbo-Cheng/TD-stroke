...
class Encoder_Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.encoder = DenseNet(inChannels=1,
                                growthRate=params['growthRate'], 
                                reduction=params['reduction'],
                                bottleneck=params['bottleneck'], 
                                use_dropout=params['use_dropout'])
        self.encoder_online = DenseNet(inChannels=2,
                                growthRate=params['growthRate'], 
                                reduction=params['reduction'],
                                bottleneck=params['bottleneck'], 
                                use_dropout=params['use_dropout'])
        self.init_context = nn.Linear(params['D'] * 2, params['n'])
    ...
    def forward(self, params, x, x_mask, C_y,  
        ...
        #encoder
        x_offline = x[:,0,:,:].unsqueeze(1)
        x_online = x[:,1:,:,:]
        ctx, ctx_mask = self.encoder(x_offline, x_mask)
        ctx_online, _ = self.encoder_online(x_online, x_mask)
        ctx_mean = (ctx * ctx_mask[:, None, :, :]).sum(3).sum(2) \
                    / ctx_mask.sum(2).sum(1)[:, None]         # (batch,D)
        ctx_mean_online = (ctx_online * ctx_mask[:, None, :, :]).sum(3).sum(2) \
                    / ctx_mask.sum(2).sum(1)[:, None]         # (batch,D)
        ctx_mean = torch.concat((ctx_mean,ctx_mean_online),dim=1)
