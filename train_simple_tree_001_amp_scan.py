...
from utils.utils import load_dict, prepare_data, prepare_data_stroke, gen_sample, weight_init, compute_wer, compute_sacc, cmp_result, load_encoder_params
from model.encoder_decoder_scane import Encoder_Decoder
...
if not os.path.exists(EXPpath):  #判断是否存在文件夹如果不存在则创建为文件夹
    os.makedirs(EXPpath)
# sys.stdout = Logger(EXPpath + '/strok.txt', sys.stdout)
# sys.stderr = Logger(EXPpath + '/strok.txt', sys.stderr)	
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
'''
找到了为什么SACC以及WER一直不涨的原因

latex的转化程序有误，或者训练数据生成有误，导致只要包含\\frac就会a导致分子分母倒置
'''
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
datasets = [scan_path + 'offline-stroke-level/data/' + 'off_stroke_img_train.pkl',   # offline img file
            scan_path + 'offline-stroke-level/data/' + 'off_stroke_mask_train.pkl', # off line mask file
            scan_path + 'online-stroke-level/data/' + 'online-stroke-train.pkl',
            scan_path + 'online-stroke-level/data/' + 'on_stroke_mask_train.pkl',
            None,
            bfs2_path + 'train_label_gtd.pkl',  # tree 
            bfs2_path + 'train_relations.pkl']  # relation

# datasets = [bfs2_path + 'train_images.pkl', bfs2_path + 'train_label_gtd.pkl', bfs2_path + 'train_relations.pkl']
# online_datasets = [bfs2_path + 'online_flow_train_feature.pkl']

# valid_datasets = [bfs2_path + '14_test_images.pkl', 
#                   bfs2_path + '14_test_label_gtd.pkl', 
#                   bfs2_path + '14_test_relations.pkl']

valid_datasets = [scan_path + 'offline-stroke-level/data/' + 'off_stroke_img_test.pkl',   # offline img file
                  scan_path + 'offline-stroke-level/data/' + 'off_stroke_mask_test.pkl',  # off line mask file
                  scan_path + 'online-stroke-level/data/' + 'online-test.pkl',
                  scan_path + 'online-stroke-level/data/' + 'on_stroke_mask_test.pkl',
                  None,
                  bfs2_path + '14_test_label_gtd.pkl', 
                  bfs2_path + '14_test_relations.pkl']

# online_valid_datasets = [bfs2_path + 'online_flow_test_feature.pkl']
...
saveto = work_path+EXPpath+'/WAP_params.pkl'
...
params['m'] = 256
params['re_m'] = 64
params['dim_attention'] = 500

params['wap_D'] = 500
params['tap_D'] = 500
params['D'] = params['tap_D'] + params['wap_D']
params['wap_reduction'] = 0.5
params['wap_bottleneck'] = True
params['tap_bottleneck'] = False
params['tap_reduction'] = 1.0
params['tap_densenet_use_dropout'] = False
params['tap_n_features'] = 488
...
params['online_input_channels'] = 9
...
train_dataIterator = BatchBucket(600, 2100, 200, 800000, 2,  #batch size 
                    datasets[0], datasets[1], datasets[2], datasets[3],datasets[4],datasets[5],datasets[6],
                    dictionaries[0], dictionaries[1])
train, train_uid = train_dataIterator.get_batches()

# valid, valid_uid = dataIterator(valid_datasets[0], valid_datasets[1], 
#                     valid_datasets[2], valid_datasets[3],valid_datasets[4],valid_datasets[5], 
#                     worddicts, reworddicts,
#                     1, 8000000, 200, 8000000)  

valid_dataIterator = BatchBucket(600, 2100, 200, 800000, 2,  #batch size 
                    valid_datasets[0], valid_datasets[1], valid_datasets[2], valid_datasets[3],valid_datasets[4],valid_datasets[5],valid_datasets[6],
                    dictionaries[0], dictionaries[1])
valid, valid_uid = valid_dataIterator.get_batches()
...
on_off_dict_params = WAP_model.state_dict()
# get all the pretrained params
tap_params_path = scan_path + 'mans-stroke-level/stroke_only_pretrained_params/' + 'TAP_params_weightnoise.pkl'
wap_params_path = scan_path + 'mans-stroke-level/stroke_only_pretrained_params/' +'WAP_params.pkl'
tap_pre_params = torch.load(tap_params_path, map_location=lambda storage, loc: storage)
wap_pre_params = torch.load(wap_params_path, map_location=lambda storage, loc: storage)
# load params(only encoder part) to on_off_modal
# update tap_encoder part
on_off_dict_params = load_encoder_params(on_off_dict_params, tap_pre_params, True)
# update wap_encoder part
on_off_dict_params = load_encoder_params(on_off_dict_params, wap_pre_params, False)
# load new params dict to on_off_model
WAP_model.load_state_dict(on_off_dict_params)
...
for eidx in range(max_epochs):
    ...
    for x, x_mask, ly, ry, re, ma, lp, rp in train:
        ...
        with autocast():
            ...
            '''
            off_x: offLine image
            x_off_mask: regular off line mask
            off_stroke_mask: mask of stroke seq
            '''
            off_x, on_x, x_off_mask, x_on_mask, off_stroke_masks, on_stroke_masks, C_y, y_mask, P_y, P_re, C_re, C_re_mask, lp, rp = \
                    prepare_data_stroke(params, x, x_mask, ly, ry, re, ma, lp, rp)

            length = C_y.shape[0] 
            off_x = torch.from_numpy(off_x).cuda()  # (batch,1,H,W)
            on_x = torch.from_numpy(on_x).cuda() # (seq_len,b,channels)
            on_x = on_x.permute(1, 2, 0)[:, :, None, :]  # (batch,9,1,seqs_x)
            
            x_off_mask = torch.from_numpy(x_off_mask).cuda()  # (batch,H,W)
            x_on_mask = torch.from_numpy(x_on_mask).cuda()  # (seq_len, batch)
            x_on_mask = x_on_mask.permute(1, 0)[:, None, :]  # (batch,1,seqs_x)
            # on_stroke_masks # len: batch, elements: (stroke, seq_len)
            # off_stroke_masks # len: batch, elements: (stroke, H, W)
            ...
            # forward
            loss, object_loss, relation_loss =  WAP_model(params, off_x, on_x, x_off_mask, x_on_mask, 
                                                          off_stroke_masks,on_stroke_masks,
        ...
        # if True:
            ...
            fp_results = open(EXPpath + '/reuslts-debug.txt', 'w')
            with torch.no_grad():
                valid_count_idx = 0
                for x, x_mask, ly, ry, re, ma, lp, rp in valid:
                    #print(len(x))
                    off_x, on_x, x_off_mask, x_on_mask, off_stroke_masks, on_stroke_masks, C_y, y_mask, P_y, P_re, C_re, C_re_mask, lp, rp = \
                    prepare_data_stroke(params, x, x_mask, ly, ry, re, ma, lp, rp, mod='valid')
                    ...
                    object_predicts, P_masks, relation_table_static, _ \
                        = WAP_model.greedy_inference(off_x, on_x, x_off_mask, x_on_mask,  
                 off_stroke_masks, on_stroke_masks, L+1, P_y[0], P_re[0], y_mask[0])
