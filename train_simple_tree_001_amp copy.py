...
train_dataIterator = BatchBucket(600, 2100, 200, 800000, 20,  #batch size 
                    datasets[0], datasets[1], datasets[2],online_datasets[0], 
                    dictionaries[0], dictionaries[1])
train, train_uid = train_dataIterator.get_batches()

valid, valid_uid = dataIterator(valid_datasets[0], valid_datasets[1], 
                    valid_datasets[2], online_valid_datasets[0], worddicts, reworddicts,
...
for eidx in range(max_epochs):
    ...
    for x, ly, ry, re, ma, lp, rp in train:
        ...
        # if True:
            ...
            with torch.no_grad():
                valid_count_idx = 0
                for x, online_x, ly, ry, re, ma, lp, rp in valid:
                    #print(len(x))
                    x, x_mask, C_y, y_mask, P_y, P_re, C_re, C_re_mask, lp, rp = \
                    prepare_data(params, (x,online_x), ly, ry, re, ma, lp, rp)
